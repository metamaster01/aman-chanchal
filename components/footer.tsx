...
const PHOTOS = [
  { src: "/images/footer/footer-1.png", rotate: -8, left: "0%" },
  { src: "/images/footer/footer-2.png", rotate: -4, left: "19%" },
  { src: "/images/footer/footer-3.png", rotate: 2, left: "38%" },
  { src: "/images/footer/footer-4.png", rotate: 7, left: "58%" },
  { src: "/images/footer/footer-5.png", rotate: -3, left: "77%" },
];
...
export function Footer() {
  return (
    <footer className="px-4 pb-6 pt-16 sm:px-8 sm:pt-20 md:pt-24 mt-14">
      {/* relative, un-clipped wrapper — photos are allowed to poke up
          above the glass panel and sit directly on the page background,
          rather than being boxed inside a giant dark card */}
      <div className="relative mx-auto max-w-7xl">
        {/* tilted polaroid strip — full width edge-to-edge, each photo
            overlapping its neighbor slightly, positioned to hang above
            and dip into the top of the glass panel below */}
        {PHOTOS.map((p, i) => (
          <motion.div
            key={p.src}
            initial={{ opacity: 0, y: 30, rotate: 0 }}
            ...
            className="absolute -top-14 w-[23%] max-w-[210px] rounded-lg bg-white p-2 shadow-2xl sm:-top-20 md:-top-34"
            style={{ left: p.left, zIndex: i + 1 }}
          >
            <div className="relative aspect-[3/4] w-full overflow-hidden rounded-md bg-neutral-200">
              <Image
        ...
        {/* frosted-glass panel — given a z-index ABOVE the photos so it
            paints on top of them where they overlap, letting
            backdrop-blur genuinely blur the photo bottoms tucked behind
            it, rather than the photos covering the text */}
        <div className="relative z-20 overflow-hidden rounded-3xl border border-white/10 bg-black/60 px-6 py-10 backdrop-blur-xl sm:px-10 sm:py-12">
          <div className="grid gap-8 sm:grid-cols-3 sm:gap-6">
            <div>
              <p className="max-w-xs text-base leading-relaxed text-white/90">
                If you&apos;re ready to launch, refresh, or reimagine your
