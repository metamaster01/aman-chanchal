...
export const WebcamPixelGrid: React.FC<WebcamPixelGridProps> = ({
  ...
  // caches the last canvas size we actually resized to, so we don't
  // reset the canvas (and its transform) on every single frame
  ...
  const render = useCallback(() => {
    ...
    // only resize (and thus reset) the canvas when its actual size
    // changed — resizing every frame is unnecessary and expensive
    if (
    ...
    // set the transform explicitly every frame (cheap) instead of
    // relying on the resize to reset it, since we now skip the resize
    // most frames
  ...
  return (
    <div className={cn("relative h-full w-full", className)}>
      ...
      {/* Error popup — `absolute` (not `fixed`) so it stays confined to
          this component's nearest positioned ancestor (the hero
          section) instead of pinning to the browser viewport corner */}
      {error && showErrorPopup && (
        <div className="animate-in fade-in slide-in-from-top-2 absolute top-4 right-4 z-50 duration-300">
      ...
      {/* Minimized error indicator — also `absolute` now, same reason */}
      {error && !showErrorPopup && (
        <button
          onClick={() => setShowErrorPopup(true)}
          className="absolute top-4 right-4 z-50 flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-black/60 text-white/50 shadow-lg backdrop-blur-xl transition-all hover:scale-105 hover:bg-black/80 hover:text-white/80"
